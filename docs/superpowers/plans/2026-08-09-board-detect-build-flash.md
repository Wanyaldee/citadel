...
All 10 tasks are implemented and committed (`a0855696ad..3e0ab8bcf9`, 19 commits). Executed via `superpowers:subagent-driven-development`: a task-scoped implementer + reviewer per task (one fix round on Task 2, an underspecified `ponytail:` comment), a final whole-branch review after Task 9 (5 Important findings, 4 fixed in one fix wave plus the plan-mandated `ponytail:` comment; 1 Important + 14 Minor findings parked — see below), then Task 10's manual hardware verification.

**Task 10 (manual hardware verification) — passed.** On an ELEGOO UNO R3 at `/dev/ttyACM0`: board auto-detected as unidentified on first connect, correctly resolved to "Arduino Uno" via the identification picker and remembered on reconnect; "Build and Upload" compiled the cached `ArduinoCore-avr` core, `cpp/io.cpp`, and `rust/`, linked, and flashed via `avrdude`; the onboard LED blink was visually confirmed. The ATmega328PB unverified-chip-warning path (Task 10 Step 6) was skipped — no second board was available during this session — per the plan's own allowance that this sub-check is "worth doing... though not build-blocking."

**A real bug was found and fixed during Task 10**, outside the normal per-task review loop (live debugging with the human, not a subagent dispatch): Citadel's own process runs under a pinned rustup toolchain (its own `rust-toolchain.toml`, stable), which sets `RUSTUP_TOOLCHAIN` in the environment. That variable overrides rustup's file-based `rust-toolchain.toml` discovery, so `citadel_build` spawning `cargo build` for a *user's* `rust/` crate silently built against Citadel's own stable toolchain instead of the project's pinned nightly, failing with "the `-Z` flag is only accepted on the nightly channel." Root-caused by reproducing with/without the inherited env var, fixed by removing it before every pipeline subprocess spawn (commit `3e0ab8bcf9`), and re-verified end to end (rebuilt, relaunched, LED blinked again). This specific fix was not run back through the automated task-reviewer/final-reviewer subagent loop — worth a lightweight look whenever this branch goes through PR review.

**Deferred / backlog (not fixed in this pass — parked during the final review, or found during manual testing, recorded here so they aren't lost now that the SDD ledger at `.superpowers/sdd/2026-08-09-board-detect-build-flash/` — gitignored scratch — eventually gets cleaned up):**

- **(Important, parked)** On startup, `BoardMonitor` unconditionally probes *every* connected serial port with `avrdude` (a DTR-resetting handshake), not just previously-identified Arduino boards — could reset an unrelated device (3D printer, GPS dongle, etc.) sharing the machine. Read-only, nothing can be bricked, but worth a deliberate UX decision (e.g. skip probing unknown/unstored VID:PIDs, or probe only on request).
- **(New, found during manual testing)** clangd doesn't know the extracted `ArduinoCore-avr` include path, and rust-analyzer's background check doesn't know about the `avr-none`/`-Z build-std=core` target — every Citadel project shows red squiggly false positives (`'Arduino.h' file not found`, duplicate `panic_impl`, etc.) in the editor even though the real build (avr-g++/cargo with the correct flags) succeeds. Cosmetic, but a real daily-use annoyance; no IDE-integration work for this was ever in scope for this plan (matches RFC 0001's own note that boundary-lint IDE integration is "still untouched").
- **(Minor, parked)** Stale file-header doc comments describing only the pre-split half of a file (`build_pipeline.rs`, `board_detect.rs`); `///` vs `//!` module-doc convention drift across 4 of 5 modules; the vendored ArduinoCore version string is coupled across three places (`.gitmodules` pin, a path literal, `-DARDUINO=10808`) with only an existence check, not a version check, so a future version bump could silently leave stale cached sources in place; `BoardKind`'s variant list is enumerated twice (`board_picker.rs`, `board_registry.rs`); the unverified-chip toast uses error/red styling for what the spec calls a warning, and fires once per open window rather than once globally; `serialport::available_ports().unwrap_or_default()` swallows enumeration errors (inherited from the plan's own spec); the poll loop's `Task<Result<()>>` uses `.detach()` rather than this repo's now-standard `.detach_and_log_err()`; `avrdude -p m328p` (signature read) vs `-p atmega328p` (flash) is inconsistent, works on avrdude ≥7.0 only; `BoardIndicator::new` uses a panicking `cx.global()` while `render()` uses `try_global()` (dead-code guard); only `cpp/io.cpp` is compiled, other `.cpp` files are silently ignored; no in-flight guard against double-clicking Build and Upload (or two open windows building against the same board) — a second concurrent `avrdude` invocation on the same port would fail confusingly rather than cleanly.
- **(Minor, parked, found 2026-08-11 on a clean Windows machine)** `board_detect.rs`'s `read_chip_signature` (the identify-on-connect probe) shells out to `avrdude` directly and does not go through `build_pipeline::check_toolchain_available()` — the design doc's "Error handling" section promises a toast for a missing toolchain, but that only covers the Build-and-Upload path. On a machine with no AVR toolchain installed at all, the identify probe instead fails with a bare `program not found` in the log (`ERROR [crates/citadel_build/src/board_detect.rs:251] program not found`) and only a generic `SignatureReadFailed` toast (no reason) reaches the user. Fix (not yet applied): run the same `check_toolchain_available()` check before the identify probe, or at least special-case `io::ErrorKind::NotFound` in `read_chip_signature`'s error path so `SignatureReadFailed` can carry a "toolchain not installed" reason.

**(New, found and fixed 2026-08-11 on a second clean Windows machine)** Three more environment-setup bugs surfaced during a fresh-clone, first-run walkthrough (`git clone` → build → New Project → Build and Upload), all fixed and hardware-verified (flash succeeded end to end on this machine):

- `read_chip_signature` and `avrdude_flash_args` (`board_detect.rs`, `build_pipeline.rs`) invoked `avrdude` by bare name via `PATH`. On this machine, the `avrdude` `PATH` entry was a symlink into a *different* installed package's directory than the one holding the matching `avrdude.conf` (an avr-gcc bundle's `avrdude.exe` sitting next to a stray, mismatched-version conf from a separate `AVRDudes.AVRDUDE` package install) — avrdude's own conf auto-discovery then silently loaded zero programmer definitions, failing every invocation with `cannot find programmer id arduino` (surfaced through `parse_signature_from_avrdude_ihex` as the more opaque `No intel-hex record found in avrdude output`). Fixed by adding `build_pipeline::locate_avrdude_conf()`, which resolves the real (symlink-followed) `avrdude` binary via `which` and passes its sibling `avrdude.conf` explicitly via `-C` at both call sites, sidestepping auto-discovery entirely.
- A plain `git clone` of this repo left the `assets/arduino-core/ArduinoCore-avr` submodule (and the `prototypes/0002-arduino-core/` one) uninitialized, so `extract_core_sources_if_needed`'s existing "was the submodule initialized?" error fired on first Build and Upload. The error message was already correct/actionable, but nothing ran the fix automatically. Fixed by adding `git submodule update --init --recursive` to the top of `script/bootstrap` and `script/bootstrap.ps1`.
- Separately (not this repo's clone, but a scaffolded *user* project's own nightly toolchain): `rustup component list` reported a scaffolded project's pinned nightly's `rust-src` as "installed" while the sysroot's `library/Cargo.lock` was actually missing on disk — a corrupted local rustup state, `rustup component remove`/`add` couldn't repair it (had to `rustup toolchain uninstall` + reinstall). This is what motivated `citadel_new_project` now eagerly running `rustup toolchain install <channel> --component rust-src` right after scaffolding (see [RFC 0002 §Project scaffolding](../../rfcs/0002-product-scope-and-dx.md#project-scaffolding)) — it won't fix a corrupted install, but it moves the failure from a confusing mid-build `E0152 duplicate lang item` to a clear, immediate toast at project-creation time.

Re-verified end to end on this machine after all three fixes: `New Project` → scaffold + git init/commit + `rustup toolchain install` all succeeded → board connected and identified → `Build and Upload` compiled and flashed successfully.

Not merged to `main` as of this writing — work sits on `rfc0001-hello-blink-prototype`.
