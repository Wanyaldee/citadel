...
function BuildZedAndItsFriends {
    Write-Output "Building Citadel and its friends, for channel: $channel"
    # Build citadel.exe, cli.exe and auto_update_helper.exe
    cargo build --release --package zed --package cli --package auto_update_helper --target $target
    Copy-Item -Path ".\$CargoOutDir\citadel.exe" -Destination "$innoDir\Citadel.exe" -Force
...
function SignZedAndItsFriends {
    ...
    $files = "$innoDir\Citadel.exe,$innoDir\cli.exe,$innoDir\auto_update_helper.exe,$innoDir\zed_explorer_command_injector.dll,$innoDir\zed_explorer_command_injector.appx"
...
function CollectFiles {
    ...
    Move-Item -Path "$innoDir\cli.exe" -Destination "$innoDir\bin\citadel.exe" -Force
    Move-Item -Path "$innoDir\zed.sh" -Destination "$innoDir\bin\citadel" -Force
...
function BuildInstaller {
    ...
    switch ($channel) {
        "stable" {
            ...
            $appName = "Citadel"
            $appDisplayName = "Citadel"
            $appSetupName = "Citadel-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "Citadel-Stable-Instance-Mutex"
            $appExeName = "Citadel"
            $regValueName = "Citadel"
            $appUserId = "CitadelProject.Citadel"
            $appShellNameShort = "C&itadel"
            $appAppxFullName = "CitadelProject.Citadel_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "preview" {
            $appId = "{{F70E4811-D0E2-4D88-AC99-D63752799F95}"
            $appIconName = "app-icon-preview"
            $appName = "Citadel Preview"
            $appDisplayName = "Citadel Preview"
            $appSetupName = "Citadel-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "Citadel-Preview-Instance-Mutex"
            $appExeName = "Citadel"
            $regValueName = "CitadelPreview"
            $appUserId = "CitadelProject.Citadel.Preview"
            $appShellNameShort = "C&itadel Preview"
            $appAppxFullName = "CitadelProject.Citadel.Preview_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "nightly" {
            $appId = "{{1BDB21D3-14E7-433C-843C-9C97382B2FE0}"
            $appIconName = "app-icon-nightly"
            $appName = "Citadel Nightly"
            $appDisplayName = "Citadel Nightly"
            $appSetupName = "Citadel-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "Citadel-Nightly-Instance-Mutex"
            $appExeName = "Citadel"
            $regValueName = "CitadelNightly"
            $appUserId = "CitadelProject.Citadel.Nightly"
            $appShellNameShort = "C&itadel Editor Nightly"
            $appAppxFullName = "CitadelProject.Citadel.Nightly_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "dev" {
            $appId = "{{8357632E-24A4-4F32-BA97-E575B4D1FE5D}"
            $appIconName = "app-icon-dev"
            $appName = "Citadel Dev"
            $appDisplayName = "Citadel Dev"
            $appSetupName = "Citadel-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "Citadel-Dev-Instance-Mutex"
            $appExeName = "Citadel"
            $regValueName = "CitadelDev"
            $appUserId = "CitadelProject.Citadel.Dev"
            $appShellNameShort = "C&itadel Dev"
            $appAppxFullName = "CitadelProject.Citadel.Dev_1.0.0.0_neutral__japxn1gcva8rg"
        }
        default {
...
if ($buildSuccess) {
    ...
    if ($Install) {
        Write-Output "Installing Citadel..."
        Start-Process -FilePath "$env:ZED_WORKSPACE/target/CitadelEditorUserSetup-x64-$env:RELEASE_VERSION.exe"
    }
    exit 0
}
else {
